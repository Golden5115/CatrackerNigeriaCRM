...
import { CheckCircle, CreditCard, Briefcase, Users, TrendingUp, AlertCircle, BarChart3, Wallet } from "lucide-react"
 ...
 // --- ADMIN DATA CRUNCHING ---
  ...
  if (user?.role === 'ADMIN') {
    ...
    allJobs.forEach(job => {
      ...
      if (job.status === 'NEW_LEAD') stats.leadsUnconverted++;
      else if (job.status === 'LEAD_LOST') stats.leadsLost++;
      else stats.leadsConverted++; 
    })
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      {/* 1. THE HERO SECTION (For Everyone) */}
        ...
        <div className="space-y-8">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
              <Wallet className="text-[#84c47c]" /> Revenue Tracking
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm border-t-4 border-t-green-400">
                <p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Today's Revenue</p>
                <h3 className="text-3xl font-bold text-gray-900">₦{stats.dailyRevenue.toLocaleString()}</h3>
              </div>
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm border-t-4 border-t-green-500">
                <p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">This Week</p>
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm border-t-4 border-t-[#2d4a2a]">
                <p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">This Month</p>
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center bg-[#e0f2de]/50 p-3 rounded-xl border border-[#84c47c]/30">
                    <span className="flex items-center gap-2 text-sm font-bold text-green-800"><TrendingUp size={16}/> Successfully Converted</span>
                  ...
                  <div className="flex justify-between items-center bg-orange-50 p-3 rounded-xl border border-orange-100">
                    <span className="flex items-center gap-2 text-sm font-bold text-orange-800"><AlertCircle size={16}/> Pending / Unconverted</span>
