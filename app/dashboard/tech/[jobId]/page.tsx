...
import { Car, User, ArrowLeft, AlertTriangle, XCircle } from "lucide-react"
import Link from "next/link"
import { revalidatePath } from "next/cache"
import TechVerificationForm from "@/components/TechVerificationForm"
...
export default async function TechVerificationPage({ 
  ...
  async function rejectJob(formData: FormData) {
    ...
    await prisma.$transaction(async (tx) => {
      // Free hardware back to inventory
      if (job?.deviceId) {
      ...
      await tx.job.update({
        ...
        data: {
          ...
          deviceId: null,        
          simCardId: null,       
          supportNotes: `TECH REJECTION: ${reason}` 
        }
      })
    })
  ...
  // Safe serialize to prevent Decimal Server-to-Client errors
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        {/* 🟢 RESTORED: Verification Form with Smart Warehouse Search */}
        <TechVerificationForm job={safeJob} />

        {/* 🟢 RESTORED: REJECTION FORM */}
        <form action={rejectJob} className="p-6 bg-red-50/50">
           ...
           <div className="flex flex-col md:flex-row gap-4">
             <input 
               ...
               className="flex-1 p-3 border border-red-200 rounded-xl bg-white focus:ring-2 focus:ring-red-500 outline-none transition shadow-sm"
             />
             <SubmitButton 
