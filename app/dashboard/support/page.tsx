...
import { Plus, CheckCircle, Clock, Trash2, Wrench, MapPin, Phone, Cpu, CreditCard, Search, Banknote } from "lucide-react"
...
export default function SupportPage() {
  ...
  async function handleResolve(e: React.FormEvent<HTMLFormElement>) {
    ...
    setImeiSearch(""); setSimSearch(""); setOldImeiSearch(""); setOldSimSearch("");
    loadData()
  }
      ...
      <div className="bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden overflow-x-auto w-full custom-scrollbar">
        <table className="min-w-full divide-y divide-gray-100">
          <thead className="bg-gray-50/50">
            <tr>
              <th className="px-6 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Client Details</th>
              <th className="px-6 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Date Initiated</th>
              <th className="px-6 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Reason / Issue</th>
              <th className="px-6 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Status & Billing</th>
              ...
              tickets.map(ticket => (
                <tr key={ticket.id} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-5">
                    ...
                    <div className="text-xs font-medium text-gray-500 mt-1 space-y-1">
                      <p className="flex items-center gap-1.5"><Phone size={12} className="text-gray-400"/> {ticket.phoneNumber}</p>
                      <p className="flex items-center gap-1.5"><MapPin size={12} className="text-gray-400"/> {ticket.address}</p>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5">
                    <p className="text-sm font-medium text-gray-700 max-w-xs truncate">{ticket.issue}</p>
                    {ticket.vehicleName && (
                      <p className="text-[10px] font-bold text-blue-600 mt-1 uppercase tracking-wider">Vehicle: {ticket.vehicleName}</p>
                    )}
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-2">
                      ...
                      {ticket.paymentStatus === 'PAID' ? (
                        <span className="bg-purple-100 text-purple-700 border border-purple-200 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest flex items-center gap-1 w-fit">
                          <Banknote size={10}/> Paid Support
                        </span>
                      ) : (
                        <span className="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest flex items-center gap-1 w-fit">
                          Free Support
                        </span>
                      )}
                    </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select name="paymentStatus" className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm font-medium bg-white">
                    <option value="FREE">Free Support</option>
                    <option value="PAID">Paid Support</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <input name="phoneNumber" required className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm font-medium" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Address / Location *</label>
                <input name="address" required className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm font-medium" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Reason for Support *</label>
                <textarea name="issue" required rows={3} placeholder="What is wrong with the tracker?" className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm font-medium resize-none"></textarea>
              </div>
            ...
            <form onSubmit={handleResolve} className="p-6 space-y-6">
              ...
              <div className="p-5 border-2 border-dashed border-red-200 bg-red-50/30 rounded-2xl relative mt-2">
                <div className="absolute -top-3 left-4 bg-red-100 border border-red-200 text-red-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
                  Old Hardware Removed (Leave blank if no swap)
                </div>
                  ...
                  <div className="relative">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="Search server or type manually..." 
              ...
              <div className="p-5 border-2 border-dashed border-gray-200 bg-gray-50 rounded-2xl relative">
                <div className="absolute -top-3 left-4 bg-gray-100 border border-gray-200 px-3 py-1 rounded-full text-[10px] font-black text-gray-500 uppercase tracking-widest">
                  New Hardware Installed (Leave blank if no swap)
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-3">
                  <div className="relative">
              ...
              <div>
                <label className="block text-[10px] font-black text-gray-500 uppercase mb-1.5 tracking-widest">Resolution Process / Fix *</label>
